    ...
    private boolean despachar(Mensaje m, PrintWriter out) {
        switch (m.getTipo()) {
            ...
            case SUSCRIBIR_PUESTO: {
                out.println(new Mensaje(TipoMensaje.OK, "SUSCRITO").serializar());
                boolean ok = coordinador.suscribirPuestoAEventos(socket);
                if (ok) {
                    return true;
                } else {
                    out.println(new Mensaje(TipoMensaje.ERROR, "SUSCRIPCION_FALLIDA").serializar());
            ...
            case HOLA_PAR: {
                ...
                if ("PRIMARIO".equals(rolPropuesto)) {
                    ...
                    out.println(new Mensaje(TipoMensaje.ERROR, "YA_HAY_PRIMARIO").serializar());
                } else {
                    out.println(new Mensaje(TipoMensaje.OK, "HOLA").serializar());
                }
                return false;
            }
            default:

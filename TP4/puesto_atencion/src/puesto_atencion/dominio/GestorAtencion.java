...
// EstadoReconexion vive en el mismo paquete (puesto_atencion.dominio).

public class GestorAtencion implements IGestorAtencion {
    ...
    public GestorAtencion(IGestorConexionServidor conexion, IHistorialLocal historial) {
    ...
    public synchronized int conectar() throws Exception {
        // Si ya teniamos un id (reconexion del proceso vivo) lo pedimos para
        // reclamar el mismo slot; si no (-1), el servidor reparte o crea uno.
        int idDeseado = (idPuesto > 0) ? idPuesto : -1;
        EstadoReconexion estado = conexion.conectarPuesto(idDeseado, null);
        aplicarEstado(estado);
        return this.idPuesto;
    }

    /** Vuelca el estado recuperado del servidor sobre el estado local del puesto. */
    private void aplicarEstado(EstadoReconexion estado) {
        this.idPuesto = estado.getIdPuesto();
        this.dniActual = estado.getDniActual();
        this.intentosActual = (estado.getDniActual() == null) ? 0 : estado.getIntentosActual();
        historial.inicializar(estado.getHistorial());
    }

...
public class ControladorConexionPuesto {
    ...
    public ControladorConexionPuesto(ConectarServidor vista, IEstrategiaEncriptacion encriptacion) {
        this.vista = vista;
    ...
    private void manejarConectar() {
        ...
        if (!ValidadorConexion.isValidIPv4(ipPrimaria)) {
            JOptionPane.showMessageDialog(vista.getFrame(), "Ingrese una IP de servidor valida (ej: 192.168.0.1).");
            return;
        }

        String textoPuertoPrimario = vista.getPuertoServidor();
        if (!ValidadorConexion.isValidPuerto(textoPuertoPrimario)) {
            JOptionPane.showMessageDialog(vista.getFrame(), "Ingrese un puerto valido (0 - 65535).");
            return;
        }
        ...
        IGestorConexionServidor conexion = new GestorConexionRedundanteServidor(direccionPrimaria, direccionSecundaria, encriptacion);
    ...
    private Direccion obtenerDireccionSecundaria(String ipSecundaria, String textoPuertoSecundario,
        ...
        if (!ipSecundaria.isEmpty() && textoPuertoSecundario.isEmpty()) {
            JOptionPane.showMessageDialog(vista.getFrame(), "Ingrese el puerto de la IP secundaria o elimine la IP secundaria.");
            return null;
        }

        if (!ValidadorConexion.isValidIPv4(ipSecundaria)) {
            JOptionPane.showMessageDialog(vista.getFrame(), "IP secundaria invalida.");
            return null;
        }

        if (!ValidadorConexion.isValidPuerto(textoPuertoSecundario)) {
            JOptionPane.showMessageDialog(vista.getFrame(), "Puerto secundario invalido (0 - 65535).");
            return null;
        }
        ...
        if (direccionSecundaria.equals(direccionPrimaria)) {
            JOptionPane.showMessageDialog(vista.getFrame(),"La direccion secundaria debe ser distinta de la primaria.");
            return null;
        }

 ...
 *
 * Un Socket TCP esta atado a UNA conexion: si el server se cae, ese socket
 * queda muerto para siempre (no se "revive"). Como el canal recuerda la
 * Direccion, puede abrir un socket NUEVO de forma transparente. Antes de
 * enviar/recibir, {@link #asegurarConexion()} reconecta si el canal esta muerto
 * o quedo marcado como roto tras un EOF/IOException previo.
 *
 */
public class CanalMensajes implements ICanalMensaje {
    ...
    private void abrirConexion() throws IOException {
        Socket s = new Socket();
        s.connect(new InetSocketAddress(direccion.getIp(), direccion.getPuerto()),
    ...
    /**
     * Reabre la conexion (new Socket) si el canal esta muerto o roto.
     * Lanza IOException si el server sigue inaccesible.
     */
    ...
    public synchronized Mensaje recibir() throws IOException {
        ...
        if (linea == null) {
            roto = true; // EOF: el server cerro; el proximo enviar reconecta
            return null;
        }

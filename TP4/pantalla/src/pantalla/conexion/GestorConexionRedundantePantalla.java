...
public class GestorConexionRedundantePantalla extends Thread {
    ...
    public GestorConexionRedundantePantalla(Direccion dirPrimaria,
        ...
        this(dirPrimaria, dirSecundaria, panelVisualizacion, null);
    }
    ...
    public void conectar() throws IOException {
        ...
        for (int i = 0; i < direcciones.size(); i++) {
            GestorConexionPantalla gestorCandidato = new GestorConexionPantalla(direcciones.get(i), panelVisualizacion, encriptacion);
            try {
        ...
        if (ultima != null)
            throw new IOException("sin nodos para conectar", ultima);
    }
        ...
        while (corriendo) {
            for (int i = 1; i <= direcciones.size(); i++) {
                ...
                for (int intentoReconectar = 0; intentoReconectar < ConfigHA.RETRY_BACKOFF_MS.length; intentoReconectar++) {
                    if (!corriendo)
                        return;
                    GestorConexionPantalla gestorCandidato = new GestorConexionPantalla(direcciones.get(indiceCandidato),
                                                                                        panelVisualizacion, 
                                                                                        encriptacion);
                    try {
                        gestorCandidato.conectar();
                        ...
                        log("intento " + (intentoReconectar + 1) + " fallido contra "
                            + direcciones.get(indiceCandidato) + ", esperando " + pausaMs + "ms");
                        try {

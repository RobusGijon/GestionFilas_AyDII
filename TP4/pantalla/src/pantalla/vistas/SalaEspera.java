...
public class SalaEspera {
    ...
    private static final Color COLOR_ACTUAL = new Color(0, 107, 63);
    ...
    private void initialize() {
        ...
        lblLlamadoActual.setFont(new Font("SansSerif", Font.BOLD, 24));
        ...
        lblHistorial.setFont(new Font("SansSerif", Font.BOLD, 24));
        ...
        configurarTabla(tablaHistorial, new Color(107, 107, 107), 22, 34, false);
    ...
    private void configurarTabla(JTable tabla, Color colorTexto, int fontSize, int rowHeight, boolean esActual) {
        ...
        tabla.setFont(new Font("SansSerif", Font.BOLD, fontSize));
    ...
    public void registrarLlamado(Mensaje respuestaServidor) {
        String dni = respuestaServidor.getDni();
        int idPuesto = respuestaServidor.getPuesto();
        if (dni != null && !dni.isEmpty() && dni.equals(llamadosActivos.get(idPuesto))) {
            return;
        }
        if (llamadosActivos.containsKey(idPuesto)) {
            String dniAnterior = llamadosActivos.get(idPuesto);
            if (java.util.Objects.equals(dniAnterior, dni)) {
                return; // re-emision del mismo llamado (replay tras reconexion): ignorar
            }
    ...
    public void quitarLlamado(Mensaje respuestaServidor) {
        int idPuesto = respuestaServidor.getPuesto();
        detenerParpadeo(idPuesto);
    ...
    public void destacarRenotificacion(Mensaje respuestaServidor) {
        int idPuesto = respuestaServidor.getPuesto();
        if (!llamadosActivos.containsKey(idPuesto)) {
            return;
        }
    ...
    /**
     * Reconstruye el historial a partir del estado que reenvia el servidor tras
     * un reinicio o reconexion. Los argumentos vienen como pares
     * {@code dni, idPuesto} en orden cronologico (mas antiguo primero). Se limpia
     * primero la tabla para que el replay sea idempotente (no duplica en
     * reconexiones HA).
     */
    public void restaurarHistorial(Mensaje respuestaServidor) {
        java.util.List<String> args = respuestaServidor.getArgs();
        modeloHistorial.setRowCount(0);
        for (int i = 0; i + 1 < args.size(); i += 2) {
            String dni = args.get(i);
            int idPuesto;
            try {
                idPuesto = Integer.parseInt(args.get(i + 1));
            } catch (NumberFormatException e) {
                continue;
            }
            agregarAHistorial(dni, idPuesto);
        }
    }

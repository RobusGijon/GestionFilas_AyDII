...
public class ControladorConexionPantalla {
    ...
    public ControladorConexionPantalla(ConectarIP vista, IEstrategiaEncriptacion encriptacion) {
        this.vista = vista;
    ...
    private void manejarConectar() {
        if (conectando) {
            return; // ya hay una conexion en curso: evita doble-suscripcion por doble clic
        }

        String ipPrimaria = vista.getIP();
        if (!ValidadorConexion.isValidIPv4(ipPrimaria)) {
            JOptionPane.showMessageDialog(vista.getFrame(), "Ingrese una IP valida (ej: 192.168.0.1).",
        ...
        GestorConexionRedundantePantalla cliente = new GestorConexionRedundantePantalla(direccionPrimaria,
                direccionSecundaria, panelVisualizacion, encriptacion);

        conectando = true;
        try {
            cliente.conectar();
        } catch (IOException ex) {
            conectando = false; // permite reintentar
            JOptionPane.showMessageDialog(vista.getFrame(),
